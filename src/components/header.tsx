    ...
    <header className="sticky top-4 z-40 w-full">
      <div className="container flex h-16 items-center justify-between bg-background/80 backdrop-blur-lg rounded-xl border shadow-sm">
        ...
        <nav className="hidden md:flex flex-1 items-center justify-center gap-6">
            <NavLinks className="text-sm font-medium text-muted-foreground hover:text-primary transition-colors" />
        </nav>

        {/* Right Aligned: Mobile Menu & Placeholder */}
        <div className="flex items-center md:flex-initial md:justify-end" style={{ minWidth: '80px' }}>
                ...
                <Sheet open={isSheetOpen} onOpenChange={setSheetOpen}>
                    ...
                    <SheetContent side="left" className="bg-background w-full max-w-xs">
                        <div className="flex flex-col p-6">
                            <Link href="/" onClick={() => setSheetOpen(false)} className="font-bold text-lg mb-8">
                              Zakaria Alikhoudja
                            </Link>
                            <div className="flex flex-col gap-6 items-start text-lg">
                               <NavLinks onLinkClick={() => setSheetOpen(false)} className="font-medium text-foreground hover:text-primary transition-colors" />
                            </div>
                        </div>
                    </SheetContent>
                </Sheet>
